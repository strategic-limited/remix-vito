...
<script>
import { vito, flatpickerSetting } from '../../../config/pluginInit'
...
export default {
  ...
  methods: {
    ...
    saveBoard (item) {
      let board = this.boards.findIndex(b => b.id === item.id)
      if (board === -1) {
    ...
    saveTask (item) {
      let index = this.board.list.findIndex(task => task.id === item.id)
      if (index !== -1) {
